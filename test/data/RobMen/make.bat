...
IF NOT EXIST %_BP%*.msi (
   ...
   %_T%light.exe %_BP%Package2.wixobj -ext WixUtilExtension -sval -o %_BP%Package2.msi
   ...
   %_T%light.exe %_BP%Package2v2.wixobj -ext WixUtilExtension -sval -o %_BP%Package2v2.msi
   )
...
IF NOT EXIST %_B%source_res\b.exe (
   ...
   %_T%light.exe %_NOTIDY% -b %_BP% -b %_M% -b %_S% -b %_T% -o %_B%source_res\b.exe %_BM%source_res.wixobj
