...
namespace BurnTestToolsUnitTests.UnitTests.BurnLogTests
{
    ...
    public class BurnLogUnitTests
    {
        ...
        public void ITBurnLogGetLatestBurnLogFromEachTempFolder()
        {
            ...
            List<BurnLog> bls = BurnLog.GetLatestBurnLogFromEachTempFolder(logFileNameHelpers, DateTime.MinValue, DateTime.MaxValue);
        ...
        public void ITBurnLogGetAllBurnLogsFromEachUsersTempFolder()
        {
            List<string> logFileNameHelpers = new List<string>();
            logFileNameHelpers.Add("*_??????????????.log");
            List<BurnLog> bls = BurnLog.GetAllBurnLogsFromEachUsersTempFolder(logFileNameHelpers, DateTime.MinValue, DateTime.MaxValue);
            Assert.IsTrue(bls.Count > 0, "No logs found");
        }
