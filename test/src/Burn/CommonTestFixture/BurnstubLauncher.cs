...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Burn.CommonTestFixture
{
    ...
    public class BurnstubLauncher
    {
        ...
        /// <summary>
        /// Creates a Burn setup.exe process (setup.exe == a loose file burn layout)
        /// </summary>
        ...
        {
            return StartProcess(layout, args, false);
        }

        /// <summary>
        /// Creates a Burn process.  It will either launch setup.exe or BurnSetupBundle.exe
        /// (setup.exe == a loose file burn layout.  BurnSetupBundle.exe == a burn bundle)
        /// </summary>
        /// <param name="layout">layout to start</param>
        /// <param name="args">args to pass to the Burn chainer</param>
        /// <param name="useBundle">true = start the BurnSetupBundle.exe, false = start setup.exe</param>
        /// <returns>Burn Process object</returns>
        public Process StartProcess(Microsoft.Tools.WindowsInstallerXml.Test.Burn.LayoutManager.LayoutManager layout, string args, bool useBundle)
        {
            string exe = System.IO.Path.Combine(layout.LayoutFolder, layout.SetupExeFilename);
            if (useBundle)
            {
                exe = System.IO.Path.Combine(layout.LayoutFolder, layout.SetupBundleFilename);
            }
        ...
        {
            string exe = null; // System.IO.Path.Combine(layout.LayoutFolder, layout.SetupExeFilename);

            string registrationId = layout.ParameterInfo.Registration.Id;
            ...
            if (UseNormalUser || layout.ParameterInfo.Registration.PerMachine != "yes")
            {
