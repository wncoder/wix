//-----------------------------------------------------------------------
// <copyright file="Bundle.RegistrySearchTests.cs" company="Microsoft">
...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.Bundle
{
    ...
    public class RegistrySearchTests : BundleTests
    {
        ...
        // bug 879792
        [TestMethod]
        [Description("RegistrySearch @Variable refrences an undefined variable.")]
        [Priority(3)]
        [Ignore]
        public void RegistrySearchUndefinedVariable()
        {
            string sourceFile = Path.Combine(RegistrySearchTests.TestDataDirectory, @"RegistrySearchUndefinedVariable\Product.wxs");
        ...
        /// <summary>
        /// Verifies RegistrySearch information in ParameterInfo.xml and Burn_Manifest.xml.
        /// </summary>
                                                            ...
                                                            string expectedExpandEnvironmentVariables, string expectedFormat)
        {
            ...
            string parameterInfoXPath = string.Format(@"//pi:RegistrySearch[@Id='{0}']", expectedId);
            XmlNodeList parameterInfoNodes = BundleTests.QueryParameterInfo(embededResourcesDirectoryPath, parameterInfoXPath);
            Assert.AreEqual(1, parameterInfoNodes.Count, "No RegistrySearch with the Id: '{0}' was found in ParameterInfo.xml.", expectedId);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "Id", expectedId);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "Root", expectedRoot);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "Key", expectedKey);
            ...
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "ExpandEnvironment", expectedExpandEnvironmentVariables);
            //BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "Format", expectedFormat);
        ...
        {
            BundleTests.VerifyParameterInfoElementOrder(embededResourcesDirectoryPath, "RegistrySearch", "Id", searchIds);
