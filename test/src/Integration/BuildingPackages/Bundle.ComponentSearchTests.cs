//-----------------------------------------------------------------------
// <copyright file="Bundle.ComponentSearchTests.cs" company="Microsoft">
...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.Bundle
{
    ...
    public class ComponentSearchTests : BundleTests
    {
        ...
        // bug 879792
        [TestMethod]
        [Description("ComponentSearch @Variable refrences an undefined variable.")]
        [Priority(3)]
        [Ignore]
        public void ComponentSearchUndefinedVariable()
        {
            string sourceFile = Path.Combine(ComponentSearchTests.TestDataDirectory, @"ComponentSearchUndefinedVariable\Product.wxs");
        ...
        /// <summary>
        /// Verifies ComponentSearch information in ParameterInfo.xml and Burn_Manifest.xml.
        /// </summary>
        ...
        public static void VerifyComponentSearchInformation(string embededResourcesDirectoryPath, string expectedId, string expectedGuid, string expectedProdcutCode, string expectedVariableName, string expectedCondition, string expectedResult)
        {
            ...
            string parameterInfoXPath = string.Format(@"//pi:MsiComponentSearch[@Id='{0}']", expectedId);
            XmlNodeList parameterInfoNodes = BundleTests.QueryParameterInfo(embededResourcesDirectoryPath, parameterInfoXPath);
            Assert.AreEqual(1, parameterInfoNodes.Count, "No ComponentSearch with the Id: '{0}' was found in ParameterInfo.xml.", expectedId);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "Id", expectedId);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "ComponentId", expectedGuid);
            BundleTests.VerifyAttributeValue(parameterInfoNodes[0], "ProductCode", expectedProdcutCode);
        ...
        {
            BundleTests.VerifyParameterInfoElementOrder(embededResourcesDirectoryPath, "MsiComponentSearch", "Id", productIds);
