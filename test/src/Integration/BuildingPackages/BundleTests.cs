//-----------------------------------------------------------------------
// <copyright file="BundleTests.cs" company="Microsoft">
...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.Bundle
{
    ...
    {
        protected const string burnManifestFileName = "bundle-manifest.xml";
        protected const string parameterInfoFileName = "bundle-paraminfo.xml";
        protected const string attachedCabFileName = "bundle-attached.cab";
        protected const string uxCabFileName = "bundle-ux.cab";
        ...
        /// <remarks>The namespace that should be used is 'pi'</remarks>
        protected static XmlNodeList QueryParameterInfo(string embededResourcesDirectoryPath, string xpathQuery)
        {
            string parameterInfoFilePath = Path.Combine(embededResourcesDirectoryPath, BundleTests.parameterInfoFileName);

            // verify that file dose exist
            Assert.IsTrue(File.Exists(parameterInfoFilePath), string.Format("ParameterInfo was not created at '{0}'.", parameterInfoFilePath));

            return Verifier.QueryParameterInfo(parameterInfoFilePath, xpathQuery);
        }
        ...
        {
            string burnManifestFilePath = Path.Combine(embededResourcesDirectoryPath, BundleTests.burnManifestFileName);
        ...
        /// <summary>
        /// Verify the order of elements in ParameterInfo.XML
        /// </summary>
        /// <param name="embededResourcesDirectoryPath">Output folder where all the embeded resources are</param>
        ...
        public static void VerifyParameterInfoElementOrder(string embededResourcesDirectoryPath, string elementName, string idAttributeName, string[] expectedOrderedValueList)
        {
            string parameterInfoXPath = string.Format(@"//pi:{0}", elementName);
            XmlNodeList parameterInfoNodes = BundleTests.QueryParameterInfo(embededResourcesDirectoryPath, parameterInfoXPath);

            BundleTests.VerifyElementOrder(parameterInfoNodes, idAttributeName, expectedOrderedValueList);
        }
