//-----------------------------------------------------------------------
// <copyright file="Bundle.PayloadTests.cs" company="Microsoft">
...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.Bundle
{
    ...
    public class PayloadTests : BundleTests
    {
        ...
        [Description("Verify that there is an error if the Payload SourceFile attribute has an invalid file name")]
        [Priority(3)]
        // bug https://sourceforge.net/tracker/?func=detail&aid=2980332&group_id=105970&atid=642714
        ...
        public void ValidPayloadDownloadURL()
        {
            ...
            // verify the ParameterInfo and burnManifest has the correct information 
            PackageTests.VerifyPackagePayloadInformation(outputDirectory, PackageTests.PackageType.MSI, "MsiPackage.msi", "PayloadFile1.txt", @"http://go.microsoft.com/fwlink/?linkid=164202", PayloadFile1);
            PackageTests.VerifyPackagePayloadInformation(outputDirectory, PackageTests.PackageType.MSI, "MsiPackage.msi", "PayloadFile2.txt", @"http://localhost/testPayload", PayloadFile2);
            PackageTests.VerifyPackagePayloadInformation(outputDirectory, PackageTests.PackageType.MSI, "MsiPackage.msi", "PayloadFile3.txt", @"ftp://192.168.0.1/testPayload.exe", PayloadFile3);
            PackageTests.VerifyPackagePayloadInformation(outputDirectory, PackageTests.PackageType.MSI, "MsiPackage.msi", "PayloadFile4.txt", @"\\wixbuild\releases\wix\", PayloadFile4);
        }
    }
}
