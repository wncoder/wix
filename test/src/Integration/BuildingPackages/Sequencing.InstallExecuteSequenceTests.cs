...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.Sequencing
{
    ...
    /// </summary>
    [TestClass]
    public class InstallExecuteSequenceTests
    {
        private static readonly string TestDataDirectory = Environment.ExpandEnvironmentVariables(@"%WIX%\test\data\Integration\BuildingPackages\Sequencing\InstallExecuteSequenceTests");

        [TestMethod]
        [Description("Verify that the InstallExecuteSequence table can be configured")]
        [Priority(1)]
        [Ignore]
