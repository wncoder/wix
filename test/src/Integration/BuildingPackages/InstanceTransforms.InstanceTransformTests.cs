//-----------------------------------------------------------------------
// <copyright file="InstanceTransforms.InstanceTransformTests.cs" company="Microsoft">
...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Integration.BuildingPackages.InstanceTransforms
{
    using System;
    ...
    {
        private static readonly string TestDataDirectory = Environment.ExpandEnvironmentVariables(@"%WIX%\test\data\Integration\BuildingPackages\InstanceTransforms\InstanceTransformTests");

        [TestMethod]
        [Description("Verify that a simple MSI with an instance transform can be built")]
        [Priority(1)]
        public void SimpleInstanceTransform()
        {
            string sourceFile = Path.Combine(InstanceTransformTests.TestDataDirectory, @"SimpleInstanceTransform\product.wxs");
            string msi = Builder.BuildPackage(sourceFile);

            // Verify that 3 instance transforms were created
            for (int i = 1; i <= 3; i++)
            {
                string transformName = String.Format("Instance{0}.mst", i);
                ...
                // Make a copy of the base MSI
                string instanceMSI = Path.Combine(Path.GetDirectoryName(msi), String.Format("{0}{1}.msi", Path.GetFileNameWithoutExtension(msi), i));
                File.Copy(msi, instanceMSI);

                // Apply the transform to the copy of the base MSI
                InstallPackage instanceMSIPackage = new InstallPackage(instanceMSI, DatabaseOpenMode.Direct);
                instanceMSIPackage.ApplyTransform(mst);
                instanceMSIPackage.Close();

                // Verify that the transformed MSI matches what is expected
                string expectedInstanceMSI = Path.Combine(InstanceTransformTests.TestDataDirectory, String.Format(@"SimpleInstanceTransform\expected{0}.msi", i));
                Verifier.VerifyResults(expectedInstanceMSI, instanceMSI);
            }

            // Verify that the base MSI matches what is expected
            Verifier.VerifyResults(Path.Combine(InstanceTransformTests.TestDataDirectory, @"SimpleInstanceTransform\expected.msi"), msi);
        }
