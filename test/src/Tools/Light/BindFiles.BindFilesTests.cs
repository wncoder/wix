...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Tools.Light.BindFiles
{
    ...
    /// </summary>
    [TestClass]
    public class BindFilesTests
    {
        private static readonly string TestDataDirectory = Environment.ExpandEnvironmentVariables(@"%WIX%\test\data\Tools\Light\BindFiles\BindFilesTests");
        ...
        public void SimpleBindFiles()
        {
            ...
            light2.OutputFile = Path.Combine(candle.OutputDirectory, "product.msi");

            // Verify that TextFile1 was not created in the layout location
            string textFile1 = Path.Combine(Path.GetDirectoryName(light2.OutputFile), @"PFiles\WixTestFolder\TextFile1.txt");
