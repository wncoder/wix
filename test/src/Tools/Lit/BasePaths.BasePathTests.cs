...
namespace Microsoft.Tools.WindowsInstallerXml.Test.Tests.Tools.Lit.BasePaths
{
    using System;
    ...
    {
        private static readonly string TestDataDirectory = Environment.ExpandEnvironmentVariables(@"%WIX_ROOT%\test\data\Tools\Lit\BasePaths\BasePathTests");

        [TestMethod]
        [Description("Verify that Lit can use a base path")]
        [Priority(1)]
        public void SimpleBasePath()
        {
            ...
            // Build the library
            Lit lit = new Lit();
            lit.ObjectFiles.Add(Candle.Compile(Path.Combine(BasePathTests.TestDataDirectory, @"SimpleBasePath\Product.wxs")));
            lit.BasePath = tempDirectory.FullName;
            lit.BindFiles = true;
            lit.Run();
