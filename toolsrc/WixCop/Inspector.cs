...
namespace Microsoft.Tools.WindowsInstaller.Tools
{
    ...
    public class Inspector
    {
        ...
        private enum InspectorTestType
        {
            ...
            /// <summary>
            /// Displayed when a an element has a LongName attribute.
            /// </summary>
            LongNameDeprecated,
            ...
            /// <summary>
            /// Displayed with a FileSearch(Ref) element has an invalid parent.
            /// </summary>
                    ...
                    if ("http://schemas.microsoft.com/wix/2005/10/sca" == node.NamespaceURI)
                    {
                        switch (node.LocalName)
                        {
                            ...
                            case "User":
                                ...
                                break;
                            default:
                                throw new InvalidOperationException(String.Format("Unknown sca extension element '{0}'.", node.LocalName));
                        }
                    }
        ...
        private void InspectComponentElement(XmlElement element)
        {
            ...
            if (null == guid)
            {
                if (this.OnError(InspectorTestType.RequireComponentGuid, guid, "The Component/@Guid attribute is required.  Setting this attribute's value to empty now enables the functionality previously represented by omitting the attribute.  This creates an unmanaged component.  Please note that unmanaged components can be a security vulnerability and should be carefully reviewed."))
                {
        ...
        private void InspectFileSearchElement(XmlElement element)
        {
            ...
            {
                if (this.OnError(InspectorTestType.FileSearchParentInvalid, element, String.Format(@"The FileSearch element is not supported under the parent element {0}. It must instead be located under a DirectorySearch element.", parentNode.LocalName)))
                {
        ...
        private void InspectPatchSequenceElement(XmlElement element)
        {
            ...
            if (null != target)
            {
                try
                {
                    Guid guid = new Guid(target.Value);
        ...
        private XmlElement InspectSqlElement(XmlElement element)
        {
            if ("http://schemas.microsoft.com/wix/SqlExtension" != element.NamespaceURI)
            {
                ...
                if (this.OnError(InspectorTestType.NamespaceChanged, element, String.Format("The {0} element is now part of the SQL extension.  An xmlns:sql=\"http://schemas.microsoft.com/wix/SqlExtension\" attribute should be added to the Wix element and this element should be renamed to 'sql:{0}'.", element.LocalName)))
                {
        ...
        private XmlElement InspectUtilElement(XmlElement element)
        {
            if ("http://schemas.microsoft.com/wix/UtilExtension" != element.NamespaceURI)
            {
                ...
                if (this.OnError(InspectorTestType.NamespaceChanged, element, String.Format("The {0} element is now part of the Utility extension.  An xmlns:util=\"http://schemas.microsoft.com/wix/UtilExtension\" attribute should be added to the Wix element and this element should be renamed to 'util:{0}'.", element.LocalName)))
                {
