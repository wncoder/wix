    ...
    internal class StronglyTypedClasses
    {
        ...
        private static void ProcessAttribute(XmlSchemaAttribute attribute, CodeTypeDeclaration typeDeclaration, CodeMemberMethod outputXmlMethod, CodeMemberMethod setAttributeMethod)
        {
            ...
            if (rawAttributeType == null || rawAttributeType.Length == 0)
            {
                ...
                bool listType = false;
                XmlSchemaSimpleTypeList simpleTypeList = attribute.AttributeSchemaType.Content as XmlSchemaSimpleTypeList;
                ...
                {
                    simpleTypeRestriction = attribute.AttributeSchemaType.Content as XmlSchemaSimpleTypeRestriction;
                }
                ...
                if (simpleTypeRestriction != null)
                {
                    ...
                    enumTypeDeclaration.CustomAttributes.Add(GetGeneratedCodeAttribute());
        ...
        private static void GenerateOutputForEnum(CodeConditionStatement fieldSetStatement, CodeConditionStatement attributeNameMatchStatement, EnumDeclaration enumDeclaration, string fieldName, string propertyName)
        {
            if (enumDeclaration.Flags)
            {
                ...
                foreach (string key in enumDeclaration.Values)
                {
                    ...
                    valueIfStatement.TrueStatements.Add(new CodeAssignStatement(new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), fieldName), new CodeBinaryOperatorExpression(new CodeFieldReferenceExpression(new CodeThisReferenceExpression(), fieldName), CodeBinaryOperatorType.BitwiseOr, new CodePropertyReferenceExpression(new CodeSnippetExpression(enumDeclaration.Name), MakeEnumValue(key)))));
