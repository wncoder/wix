                    ...
                    if (appInfo != null)
                    {
                        foreach (XmlNode node in appInfo.Markup)
                        {
                            ...
                            if (element != null && element.LocalName == "msiRef" && element.NamespaceURI == XmlSchemaExtensionNamespace)
                            {
                                ...
                                {
                                    WriteLink(String.Format("http://msdn.microsoft.com/library/default.asp?url=/library/en-us/msi/setup/{0}_action.asp?frame=true&hidetoc=true", action.ToLower()), String.Concat(action, " Action"), null, writer);
                                }
