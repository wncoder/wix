...
{
    AssertSz(INVALID_HANDLE_VALUE == LogUtil_hLog && !LogUtil_sczLogPath, "LogInitialize() - already called");
    HRESULT hr = S_OK;
    LogUtil_hModule = hModule;

    if (wzExt && *wzExt)
    {
        hr = FileCreateTemp(wzLog, wzExt, &LogUtil_sczLogPath, &LogUtil_hLog);
        ExitOnFailure2(hr, "failed to create log for: %ls.%ls", wzLog, wzExt);
    }
    else
    {
        LogUtil_hLog = ::CreateFileW(wzLog, GENERIC_WRITE, FILE_SHARE_READ, NULL, (fAppend) ? OPEN_ALWAYS : CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (INVALID_HANDLE_VALUE == LogUtil_hLog)
        {
            ExitOnLastError1(hr, "failed to create log file: %ls", wzLog);
        }
        ...
        {
            // TODO: Write UTF-8 BOM here?
        }

        hr = StrAllocString(&LogUtil_sczLogPath, wzLog, 0);
        ExitOnFailure1(hr, "failed to allocate memory for log: %ls", wzLog);
    }
    ...
    {
        // Log anything that was logged before LogInitialize() was called
...
extern "C" HRESULT DAPI LogHeader()
{
    ...
    StrCurrentDateTime(&sczCurrentDateTime, FALSE);
    ...
    )
{
    ...
    if (fLOGUTIL_NEWLINE)
    {
        ...
        hr = StrAnsiAllocFormatted(&scz, "%s[%04hu-%02hu-%02huT%02hu:%02hu:%02hu.%03hu%c%02u:%02u]:%s %s%s", LogUtil_sczSpecialBeginLine ? LogUtil_sczSpecialBeginLine : "",
            st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, 0 >= tzi.Bias ? L'+' : L'-', dwAbsBias / 60, dwAbsBias % 60,
