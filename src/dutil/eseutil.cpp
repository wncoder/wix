//-------------------------------------------------------------------------------------------------
// <copyright file="eseutil.cpp" company="Microsoft">
...
#define ExitOnJetError(e, x, s) { x = HresultFromJetError(e); if (S_OK != x) { Dutil_RootFailure(__FILE__, __LINE__, x); ExitTrace(x, s); goto LExit; }}
#define ExitOnJetError1(e, x, f, s) { x = HresultFromJetError(e); if (S_OK != x) { Dutil_RootFailure(__FILE__, __LINE__, x); ExitTrace1(x, f, s); goto LExit; }}
#define ExitOnJetError2(e, x, f, s, t) { x = HresultFromJetError(e); if (S_OK != x) { Dutil_RootFailure(__FILE__, __LINE__, x); ExitTrace(x, f, s, t); goto LExit; }}
#define ExitOnJetError3(e, x, f, s, t, u) { x = HresultFromJetError(e); if (S_OK != x) { Dutil_RootFailure(__FILE__, __LINE__, x); ExitTrace1(x, f, s, t, u); goto LExit; }}
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to create instance");

    jEr = JetSetSystemParameter(pjiInstance, NULL, JET_paramSystemPath, NULL, pszAnsiPath);
    ExitOnJetError1(jEr, hr, "Failed to set jet system path to: %s", pszAnsiPath);
    ...
    ExitOnJetError1(jEr, hr, "Failed to set jet log file path to: %s", pszAnsiPath);

    jEr = JetSetSystemParameter(pjiInstance, NULL, JET_paramMaxOpenTables, 10, NULL);
    ExitOnJetError(jEr, hr, "Failed to set jet max open tables parameter");
    ...
    ExitOnJetError(jEr, hr, "Failed to initialize jet engine instance");

    jEr = JetBeginSession(*pjiInstance, pjsSession, NULL, NULL);
    ExitOnJetError(jEr, hr, "Failed to begin jet session");
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to end jet session");

    jEr = JetTerm(jiInstance);
    ExitOnJetError(jEr, hr, "Failed to uninitialize jet engine instance");
    ...
    )
{
    ...
    for (dwTable = 0;dwTable < pdsSchema->dwTables; dwTable++)
    {
        ...
        if (E_NOTFOUND == hr) // if the table is missing, create it
        {
            ...
            ExitOnJetError1(jEr, hr, "Failed to create %S table", pwzTableName);
    ...
    )
{
    ...
    if (FileExistsEx(pszFile, NULL))
    {
        ...
        ExitOnJetError1(jEr, hr, "Failed to attach to database %s", pszAnsiFile);
        ...
        ExitOnJetError1(jEr, hr, "Failed to open database %s", pszAnsiFile);
    }
    else
    {
        jEr = JetCreateDatabase2A(jsSession, pszAnsiFile, 0, pjdbDb, 0);
        ExitOnJetError1(jEr, hr, "Failed to create database %S", pszFile);
    }
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to close database");
    ...
    )
{
    ...
    ExitOnJetError1(jEr, hr, "Failed to create table %s", pszAnsiTable);
    ...
    )
{
    ...
    ExitOnJetError1(jEr, hr, "Failed to open table %s", pszAnsiTable);
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to close table");
    ...
    )
{
    ...
    ExitOnJetError1(jEr, hr, "Failed to check if column exists: %s", pszAnsiColumnName);
    ...
    ExitOnJetError1(jEr, hr, "Failed to add column %S", pszColumnName);
    ...
    )
{
    ...
    ExitOnJetError1(jEr, hr, "Failed to move jet cursor by amount: %d", lRow);
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to delete row");
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to begin transaction");
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to commit transaction");
    ...
    )
{
    ...
    ExitOnJetError1(jEr, hr, "Failed to prepare for update of type: %ul", ulPrep);
    ...
    )
{
    ...
    ExitOnJetError(jEr, hr, "Failed to run update");
    ...
    )
{
    ...
    jEr = JetSetColumn(jsSession, jtTable, jcColumn, &dwValue, sizeof(DWORD), 0, NULL);
    ExitOnJetError1(jEr, hr, "Failed to set dword value into column of database: %d", dwValue);
    ...
    )
{
    ...
    hr = EsePrepareUpdate(jsSession, jtTable, JET_prepReplace);
    ExitOnFailure(hr, "Failed to prepare for replace");

    hr = EseSetColumnDword(jsSession, jtTable, jcColumn, dwValue);
    ExitOnFailure1(hr, "Failed to set column string value: %d", dwValue);

    // This finishes the row
    hr = EseFinishUpdate(jsSession, jtTable);
    ...
    )
{
    ...
    jEr = JetSetColumn(jsSession, jtTable, jcColumn, pwzValue, cbValueSize, 0, NULL);
    ExitOnJetError1(jEr, hr, "Failed to set string value into column of database: %S", pwzValue);
    ...
    )
{
    ...
    hr = EseSetColumnString(jsSession, jtTable, jcColumn, pszValue);
    ...
    )
{
    ...
    jEr = JetSetColumn(jsSession, jtTable, jcColumn, NULL, 0, 0, NULL);
    ExitOnJetError(jEr, hr, "Failed to set empty value into column of database");
    ...
    )
{
    ...
    hr = EseSetColumnEmpty(jsSession, jtTable, jcColumn);
    ...
    )
{
    ...
    jEr = JetRetrieveColumn(jsSession, jtTable, jcColumn, pdwValue, sizeof(DWORD), NULL, 0, NULL);
    ExitOnJetError(jEr, hr, "Failed to retrieve dword value from record");
    ...
    )
{
    ...
    jEr = JetRetrieveColumn(jsSession, jtTable, jcColumn, NULL, 0, &ulActualSize, 0, NULL);
    if (JET_wrnBufferTruncated == jEr)
    {
        jEr = JET_errSuccess;
    }
    ExitOnJetError(jEr, hr, "Failed to check size of string value from record");

    hr = StrAlloc(ppszValue, ulActualSize);
    ExitOnFailure(hr, "Failed to allocate string while retrieving column value");

    jEr = JetRetrieveColumn(jsSession, jtTable, jcColumn, *ppszValue, ulActualSize, NULL, 0, NULL);
    ExitOnJetError(jEr, hr, "Failed to retrieve string value from record");
    ...
    )
{
    ...
    jEr = JetMakeKey(jsSession, jtTable, pvData, cbData, jGrb);
    ExitOnJetError(jEr, hr, "Failed to begin new query");
    ...
    )
{
    ...
    hr = SetQueryColumn(jsSession, jtTable, eqhHandle, (const void *)&dwData, sizeof(DWORD), jGrb);
    ...
    )
{
    ...
    hr = SetQueryColumn(jsSession, jtTable, eqhHandle, (const void *)pszString, dwStringSize, jGrb);
    ...
    )
{
    ...
    const ESE_QUERY *peqHandle = static_cast<const ESE_QUERY *>(eqhHandle);

    if (peqHandle->fExact)
    {
        jEr = JetSeek(jsSession, jtTable, JET_bitSeekEQ);
        ExitOnJetError(jEr, hr, "Failed to seek EQ within jet table");
    }
    else
    {
        jEr = JetSeek(jsSession, jtTable, JET_bitSeekGE);
        ...
        for (i = 0; i < peqHandle->dwColumns; i++)
        {
            ...
            jEr = JetMakeKey(jsSession, jtTable, peqHandle->pvData[i], peqHandle->cbData[i], jGrb);
            ExitOnJetError(jEr, hr, "Failed to begin new query");
        }

        jEr = JetSetIndexRange(jsSession, jtTable, JET_bitRangeUpperLimit);
        ExitOnJetError(jEr, hr, "Failed to set index range");
    }
