    ...
    )
{
    ...
    {
        hr = ERROR_MORE_DATA;
        ExitOnFailure1(hr, "Buffer size is not big enough to hold domain name: %S", pwzDomain);
    }
    else if (cch > 0)
    {
    ...
    {
        hr = ERROR_MORE_DATA;
        ExitOnFailure1(hr, "Buffer size is not big enough to hold user name: %S", pwzName);
    }
    ...
    )
{
    ...
    ExitOnFailure1(hr, "Failed to get list of global groups for user while checking group membership information for user: %S", pwzName);

    if (dwRead != dwTotal)
    {
        hr = HRESULT_FROM_WIN32(ERROR_MORE_DATA);
        ExitOnFailure1(hr, "Failed to get entire list of groups (global) for user while checking group membership information for user: %S", pwzName);
    }
    ...
    hr = HRESULT_FROM_WIN32(er);
    ExitOnFailure1(hr, "Failed to get list of groups for user while checking group membership information for user: %S", pwzName);

    if (dwRead != dwTotal)
    {
        hr = HRESULT_FROM_WIN32(ERROR_MORE_DATA);
        ExitOnFailure1(hr, "Failed to get entire list of groups (local) for user while checking group membership information for user: %S", pwzName);
    }
