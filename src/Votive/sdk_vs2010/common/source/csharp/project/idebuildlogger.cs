...
namespace Microsoft.VisualStudio.Package
{
	...
	internal sealed class IDEBuildLogger : Logger
	{
		...
		private IVsOutputWindowPane2 outputWindowPane2;
		...
		internal IVsOutputWindowPane OutputWindowPane
		{
			...
			set
			{
				this.outputWindowPane = value;
				this.outputWindowPane2 = value as IVsOutputWindowPane2;
			}
		}
		#endregion
		...
		public IDEBuildLogger(IVsOutputWindowPane output, TaskProvider taskProvider, IVsHierarchy hierarchy)
		{
			...
			this.outputWindowPane2 = output as IVsOutputWindowPane2;
			...
			int column)
		{
				...
				&& (this.Verbosity != LoggerVerbosity.Quiet || errorEvent is BuildErrorEventArgs))
			{
				...
				string formattedMessage = GetFormattedErrorMessage(e);

				if (outputWindowPane2 != null)
				{
					outputWindowPane2.OutputTaskItemStringEx2(
						formattedMessage,
						(isWarning) ? VSTASKPRIORITY.TP_NORMAL : VSTASKPRIORITY.TP_HIGH,
						VSTASKCATEGORY.CAT_BUILDCOMPILE,
						String.Empty,
						(int)_vstaskbitmap.BMP_COMPILE,
						file,
						line > 0 ? (uint)line - 1 : 0,
						(uint)column,
						String.Empty,
						errorEvent.Message,
						String.Empty);
				...
				{
					Microsoft.VisualStudio.ErrorHandler.ThrowOnFailure(OutputWindowPane.OutputStringThreadSafe(formattedMessage));
				}
