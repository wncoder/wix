...
namespace WixToolset.Data
{
    ...
    public sealed class SymbolCollection : ICollection
    {
        ...
        public void AddDuplicate(Symbol symbol)
        {
            ...
            if (null == o)
            {
                throw new InvalidOperationException(WixDataStrings.EXP_DidnotFindDuplicateSymbol);
            }
            else
            {
        ...
        public Symbol GetSymbolForSimpleReference(WixSimpleReferenceRow wixSimpleReferenceRow, IMessageHandler messageHandler)
        {
            Symbol symbol = null;
            ...
            catch (DuplicateSymbolsException e)
            {
                ...
                foreach (SourceLineNumber sourceLineNumbers in uniqueSourceLineNumbers.Keys)
                {
                    ...
                    {
                        messageHandler.OnMessage(WixDataErrors.DuplicateSymbol(sourceLineNumbers, duplicateSymbols[0].Name));
                        displayedFirstError = true;
                    }
                    else
                    {
                        messageHandler.OnMessage(WixDataErrors.DuplicateSymbol2(sourceLineNumbers));
                    }
                }
                ...
                if (!displayedFirstError)
                {
                    messageHandler.OnMessage(WixDataErrors.DuplicateSymbol(null, duplicateSymbols[0].Name));
                }
            }
        ...
        public void OutputSymbols(string outputPath)
        {
            if (null == outputPath)
            {
