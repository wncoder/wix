...
namespace WixToolset.Data
{
    ...
    public sealed class SectionCollection : ICollection
    {
        ...
        public Dictionary<string, List<Symbol>> GetDuplicateSymbols(IMessageHandler messageHandler)
        {
        ...
        public SymbolCollection GetOrphanedSymbols(StringCollection referencedSymbols, IMessageHandler messageHandler)
        {
        ...
        public void FindEntrySectionAndLoadSymbols(
            ...
            out SymbolCollection allSymbols)
        {
            ...
            foreach (Section section in this.collection.Keys)
            {
                if (SectionType.Product == section.Type || SectionType.Module == section.Type || SectionType.PatchCreation == section.Type || SectionType.Patch == section.Type || SectionType.Bundle == section.Type)
                {
                    ...
                    {
                        messageHandler.OnMessage(WixDataWarnings.UnexpectedEntrySection(section.SourceLineNumbers, section.Type.ToString(), expectedEntrySectionType.ToString(), outputExtension));
                    }
                    ...
                    {
                        messageHandler.OnMessage(WixDataErrors.MultipleEntrySections(entrySection.SourceLineNumbers, entrySection.Id, section.Id));
                        messageHandler.OnMessage(WixDataErrors.MultipleEntrySections2(section.SourceLineNumbers));
                    }
                }
                ...
                foreach (Symbol symbol in section.GetSymbols(messageHandler))
                {
                    try
                    {
                        ...
                        {
                            messageHandler.OnMessage(WixDataWarnings.IdenticalRowWarning(symbol.Row.SourceLineNumbers, existingSymbol.Name));
                            messageHandler.OnMessage(WixDataWarnings.IdenticalRowWarning2(existingSymbol.Row.SourceLineNumbers));
                        }
                        else
                        {
