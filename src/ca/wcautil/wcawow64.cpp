...
extern "C" HRESULT WIXAPI WcaInitializeWow64()
{
    ...
    if (!s_hKernel32)
    {
        ExitOnLastError(hr, "failed to get handle to kernel32.dll");
    }
    ...
    if (NULL != GetProcAddress(s_hKernel32,"IsWow64Process"))
    {
        ...
        {
            WcaLog(LOGMSG_STANDARD, "Found Wow64 API, but unable to link to Wow64DisableWow64FsRedirection function in kernel32.dll");
        }

        s_pfnRevertWow64 = (BOOL (*)(PVOID))GetProcAddress(s_hKernel32, "Wow64RevertWow64FsRedirection");
        if (!s_pfnRevertWow64)
        {
            WcaLog(LOGMSG_STANDARD,"Found Wow64 API, but unable to link to Wow64RevertWow64FsRedirection function in kernel32.dll");
        }
