//-------------------------------------------------------------------------------------------------
// <copyright file="cpiasmsched.cpp" company="Microsoft Corporation">
	...
	)
{
	...
	LPWSTR pwzProcessorArchitecture = NULL;
	LPWSTR pwzLanguage = NULL;
	...
	while (S_OK == (hr = WcaFetchRecord(hView, &hRec)))
	{
		...
		else if (0 == lstrcmpiW(L"processorArchitecture", pwzKey))
			hr = WcaGetRecordString(hRec, manqValue, &pwzProcessorArchitecture);
		else if (0 == lstrcmpiW(L"language", pwzKey))
			hr = WcaGetRecordString(hRec, manqValue, &pwzLanguage);
		else
		{
	...
	// build name string
	hr = StrAllocFormatted(ppwzAssemblyName, L"%s, Version=%s, Culture=%s, PublicKeyToken=%s, ProcessorArchitecture=%s, Language=%s",
		pwzName, pwzVersion,
		pwzCulture && *pwzCulture ? pwzCulture : L"Neutral",
		pwzPublicKeyToken && *pwzPublicKeyToken ? pwzPublicKeyToken : L"null",
		pwzProcessorArchitecture && *pwzProcessorArchitecture ? pwzProcessorArchitecture : L"MSIL",
		pwzLanguage && *pwzLanguage ? pwzLanguage : L"Neutral");
...
LExit:
	...
	ReleaseStr(pwzProcessorArchitecture);
	ReleaseStr(pwzLanguage);
