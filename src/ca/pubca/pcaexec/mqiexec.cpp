//-------------------------------------------------------------------------------------------------
// <copyright file="mqiexec.cpp" company="Microsoft Corporation">
	...
	)
{
	...
	// create message queue
	hr = gpfnMQCreateQueue(NULL, &props, NULL, &dwFormatNameLength);
	...
	)
{
	...
	do {
		...
		{
			psd = ::HeapReAlloc(::GetProcessHeap(), HEAP_ZERO_MEMORY, psd, dw);
