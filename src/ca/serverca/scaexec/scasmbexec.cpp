...
HRESULT AllocateAcl(SCA_SMBP* pssp, PACL* ppACL)
{
	...
	pEA = (EXPLICIT_ACCESSW*)MemAlloc(cEA * sizeof(EXPLICIT_ACCESSW), TRUE);
	...
	for (dwCounter = 0; dwCounter < pssp->dwUserPermissionCount; dwCounter++)
	{
		...
		*/
		...
		pEA[dwCounter].grfAccessMode = SET_ACCESS;
...
********************************************************************/
BOOL DoesShareExist(__in LPWSTR wzShareName)
{
	BOOL fRet = TRUE;
	NET_API_STATUS s;
	...
	if (s == NERR_NetNameNotFound)
		fRet = FALSE;
	else if (s != NERR_Success)
		WcaLogError(s, "FShareExists return value unexpected.", NULL);
...
HRESULT CreateShare(SCA_SMBP* pssp)
{
	...
	BOOL fShareExists = DoesShareExist(pssp->wzKey);

	PSECURITY_DESCRIPTOR pSD = (PSECURITY_DESCRIPTOR)MemAlloc(SECURITY_DESCRIPTOR_MIN_LENGTH, TRUE);
