...
enum eCertQuery { cqCertificate = 1, cqName, cqComponent, cqStoreLocation, cqStoreName, cqAttributes, cqBinary, cqCertificatePath, cqPFXPassword };
    ...
    )
{
    ...
    while (SUCCEEDED(hr = WcaFetchRecord(hViewCertificate, &hRecCertificate)))
    {
        ...
        {
            hr = WcaGetRecordString(hRecCertificate, cqBinary, &pwzData);
    ...
    )
{
    ...
    while (pCertContext = ::CertFindCertificateInStore(hCertStore, PKCS_7_ASN_ENCODING | X509_ASN_ENCODING, 0, CERT_FIND_ANY, NULL, pCertContext))
    {
