        ...
        private int Run(string[] args)
        {
            try
            {
                ...
                else if (null == this.outputFile)
                {
                    ...
                    this.outputFile = Path.ChangeExtension(Path.GetFileName(this.inputFiles[0]), ".wix");
                }
                ...
                if (null != library)
                {
                    if (this.bindFiles)
                    {
                        ...
                        foreach (string basePath in this.basePaths)
                        {
                            this.binderFileManager.BasePaths.Add(basePath);
                        }
        ...
        private void ParseCommandLine(string[] args)
        {
            for (int i = 0; i < args.Length; ++i)
            {
                ...
                if ('-' == arg[0] || '/' == arg[0])
                {
                    ...
                    if ("b" == parameter)
                    {
                        string basePath = CommandLine.GetDirectory(parameter, this.messageHandler, args, ++i);
