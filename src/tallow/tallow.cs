...
namespace Microsoft.Tools.WindowsInstallerXml.Tools
{
    ...
    public class Tallow
    {
        ...
        internal class TallowMain
        {
            ...
            private UIntPtr regRootToOverride = HkeyLocalMachine;
            ...
            public TallowMain(string[] args)
            {
                ...
                if (0 == this.comregAssemblies.Count && 0 == this.dirwalkDirectories.Count && 0 == this.selfregFiles.Count && 0 == this.resourceFiles.Count && 0 == this.registryFiles.Count)
                {
                    this.showHelp = true;
                }
                ...
                if (this.showHelp)
                {
                    Console.WriteLine(" usage:  tallow.exe [-?] [-nologo] [-1] [-c assembly] [-s file]");
                ...
                if (0 < this.comregAssemblies.Count || 0 < this.dirwalkDirectories.Count || 0 < this.selfregFiles.Count)
                {
                    ...
                    if (majorOSVersion >= 6)
                    {
                        ...
                        regRootToOverride = HkeyCurrentUser;
                    }
            ...
            private void WriteSubKeys(XmlWriter writer, RegistryKey key)
            {
                ...
                // Remove the sub-string "HKEY_CURRENT_USER\SOFTWARE\WiX\" or
                // "HKEY_LOCAL_MACHINE\SOFTWARE\WiX\" from the beginning of the key
                string path = key.Name.Substring(regKeyToOverride.Name.Length + 14);
            ...
            /// <summary>
            /// Begins overriding a registry hive with a key under HKLM or HKCU.
            /// </summary>
            ...
            private void StartOverridingRegHive(UIntPtr hive, string regOverride)
            {
                ...
                try
                {
                    key = NativeMethods.OpenRegistryKey(regRootToOverride, regOverride);
            ...
            private void StartOverridingRegHives()
            {
                ...
                // override the registry roots supported by MSI
                ...
                // to be overridden under HKLM\Software\WiX\HKLM\Software\WiX\HKCR instead
                // of under HKLM\Software\WiX\HKCR
                if (majorOSVersion < 6)
                {
                ...
                else
                {
                    ...
                    // HKLM\Software\Classes\Typelib exist, so add it to the remapped root
            ...
            private void EndOverridingRegHive(UIntPtr hive, XmlTextWriter writer, string regOverride)
            {
                ...
                if (null != writer)
                {
                    ...
                    using (RegistryKey regKey = regKeyToOverride.OpenSubKey(regOverride))
                    {
            ...
            private void EndOverridingRegHives(XmlTextWriter writer)
            {
                ...
                if (majorOSVersion < 6)
                {
                    EndOverridingRegHive(HkeyLocalMachine, writer, @"SOFTWARE\WiX\HKLM");
            ...
            private void RemovePrivateRegKey()
            {
                try
                {
                    regKeyToOverride.DeleteSubKeyTree("SOFTWARE\\WiX");
                }
                catch (ArgumentException)
                {
                    // ignore the error case where the "private registry" key does not exist
                }
            }
