    ...
    )
{
    ...
    case BURN_SYMBOL_TYPE_IDENTIFIER:
        ...
        if (E_NOTFOUND != hr)
        {
            ExitOnFailure(hr, "Failed to find variable.");
        }
        break;
    ...
    default:
        hr = Error(pContext);
        ExitOnFailure(hr, "Failed to report error.");
    ...
    )
{
    ...
    Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Unexpected symbol at position %d.", pContext->wzCondition, pContext->NextSymbol.iPosition);
    return S_OK;
}
    ...
    default:
        if (C1_DIGIT & charType || L'-' == pContext->wzRead[0])
        {
            do
            {
                ...
                if (C1_ALPHA & charType || L'_' == pContext->wzRead[n])
                {
                    ...
                    Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Identifier cannot start at a digit, at position %d.", pContext->wzCondition, iPosition);
                    pContext->fError = TRUE;
                    ExitFunction1(hr = E_FAIL);
                }
            ...
            if (FAILED(hr))
            {
                Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Constant too big, at position %d.", pContext->wzCondition, iPosition);
                pContext->fError = TRUE;
                ExitFunction();
            }
        ...
        else if (C1_ALPHA & charType || L'_' == pContext->wzRead[0])
        {
            ...
            if (L'v' == pContext->wzRead[0] && C1_DIGIT & charType)
            {
                ...
                for (;;)
                {
                    ...
                    if (L'.' == pContext->wzRead[n])
                    {
                        ...
                        if (4 < cParts)
                        {
                            ...
                            Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Version can have a maximum of 4 parts, at position %d.", pContext->wzCondition, iPosition);
                            pContext->fError = TRUE;
                            ExitFunction1(hr = E_FAIL);
                        }
                    }
                ...
                if (FAILED(hr))
                {
                    Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Invalid version format, at position %d.", pContext->wzCondition, iPosition);
                    pContext->fError = TRUE;
                    ExitFunction();
                }
        ...
        else
        {
            ...
            Trace2(REPORT_STANDARD, "Failed to parse condition \"%S\". Unexpected character at position %d.", pContext->wzCondition, iPosition);
            pContext->fError = TRUE;
            ExitFunction1(hr = E_FAIL);
        }
    }
