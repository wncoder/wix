...
static HRESULT CopyFileCallback(
    __in BURN_CONTAINER_CONTEXT* pContext,
    __inout FDINOTIFICATION *pFDINotify
    );
static HRESULT CloseFileInfoCallback(
    __in BURN_CONTAINER_CONTEXT* pContext,
    __inout FDINOTIFICATION *pFDINotify
    );
    ...
    )
{
    ...
    case WAIT_OBJECT_0 + 1:
        ...
        ExitOnFailure(hr, "Extraction thread terminated unexpectedly.");
        break;
    ...
    )
{
    ...
    {
        if (FAILED(pContext->Cabinet.hrError))
        {
            hr = pContext->Cabinet.hrError;
        }
        else if (ERROR_SUCCESS != erf.erfType)
        {
        ...
        ExitOnFailure(hr, "Failed to extract all files from compressed payload.");
    }
    ...
    )
{
    ...
    {
    case fdintCOPY_FILE:
        hr = CopyFileCallback(pContext, pFDINotify);
        ipResult = 1;
        break;

    case fdintCLOSE_FILE_INFO: // resource extraction complete
        hr = CloseFileInfoCallback(pContext, pFDINotify);
        ipResult = 1;
        break;
    ...
    )
{
    ...
    case BURN_CAB_OPERATION_STREAM_TO_FILE:
        ...
        // set file size
        if (!::SetFilePointer(hFile, pFDINotify->cb, NULL, FILE_BEGIN))
        {
