...
namespace Microsoft.Tools.WindowsInstallerXml
{
    ...
    public sealed class Compiler
    {
        ...
        private void ParseBundleElement(XmlNode node)
        {
            ...
            this.activeName = Common.GenerateGuid();
            ...
            if (!chainSeen)
            {
                this.core.OnMessage(WixErrors.ExpectedElement(sourceLineNumbers, node.Name, "Chain"));
            }
        }
        ...
        private void ParseContainerElement(XmlNode node)
        {
            ...
            {
                containerId = CompilerCore.GetIdentifierFromName(name);
            }
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (XmlNodeType.Element == child.NodeType)
                {
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        switch (child.LocalName)
                        {
                            // TODO: Containers will have PackageGroupRefs, not direct Payloads.
                            ////case "Payload":
                            ////    this.ParsePayloadElement(child, ComplexReferenceParentType.Unknown, containerId);
                            ////    break;
                            default:
                                this.core.UnexpectedElement(node, child);
                                break;
        ...
        private void ParseUXElement(XmlNode node)
        {
            ...
            string previousId = this.ParsePayloadElementAttributes(node, ComplexReferenceParentType.PayloadGroup, "WixUXContainer", ComplexReferenceChildType.Unknown, null, false);
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (XmlNodeType.Element == child.NodeType)
                {
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        ...
                        {
                            case "Payload":
                                previousId = this.ParsePayloadElement(child, ComplexReferenceParentType.PayloadGroup, "WixUXContainer", previousType, previousId);
                                previousType = ComplexReferenceChildType.Payload;
                                break;
                            case "PayloadGroupRef":
                                previousId = this.ParsePayloadGroupRefElement(child, ComplexReferenceParentType.PayloadGroup, "WixUXContainer", previousType, previousId);
            ...
            if (null == previousId)
            {
                // We need *either* <Payload> or <PayloadGroupRef> or even just @SourceFile on the UX...
            ...
            // Note that we *don't* create a UX table entry, because all we
            // need to know is stored in the WixGroup table.
        }
        ...
        /// </summary>
        /// <param name="node">Element to parse</param>
        /// <param name="parentType">ComplexReferenceParentType of parent element. (UX or PayloadGroup)</param>
        /// <param name="parentId">Identifier of parent element.</param>
        ...
        {
            Debug.Assert(ComplexReferenceParentType.PayloadGroup == parentType || ComplexReferenceParentType.Package == parentType);
        ...
        private string ParsePayloadElementAttributes(XmlNode node, ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId, bool required)
        {
            ...
            CreatePayloadRows(sourceLineNumbers, id, name, sourceFile, downloadUrl, parentType, parentId, previousType, previousId);

            return id;
        }

        /// <summary>
        /// Creates the rows for a Payload.
        /// </summary>
        /// <param name="node">Element to parse</param>
        /// <param name="parentType">ComplexReferenceParentType of parent element. (UX or PayloadGroup)</param>
        /// <param name="parentId">Identifier of parent element.</param>
        private void CreatePayloadRows(SourceLineNumberCollection sourceLineNumbers, string id, string name, string sourceFile, string downloadUrl,
            ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId)
        {
            if (!this.core.EncounteredError)
            {
            ...
            foreach (XmlAttribute attrib in node.Attributes)
            {
                if (0 == attrib.NamespaceURI.Length || attrib.NamespaceURI == this.schema.TargetNamespace)
                {
                    switch (attrib.LocalName)
                    {
                        case "Id":
                            id = this.core.GetAttributeIdentifierValue(sourceLineNumbers, attrib);
                            this.core.CreateWixSimpleReferenceRow(sourceLineNumbers, "PayloadGroup", id);
                            break;
                        default:
                            this.core.UnexpectedAttribute(sourceLineNumbers, attrib);
                            break;
        ...
        private void ParseChainElement(XmlNode node)
        {
            ...
            if (null == previousId)
            {
                this.core.OnMessage(WixErrors.ExpectedElement(sourceLineNumbers, node.Name, "MsiPackage", "ExePackage", "PackageGroupRef"));
            }
        }
        ...
        private string ParseMsiPackageElement(XmlNode node, ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId)
        {
        ...
        /// <param name="previousId">Identifier of previous item, if known</param>
        /// <returns>Identifier for package element.</returns>
        private string ParseMspPackageElement(XmlNode node, ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId)
        {
        ...
        /// <param name="previousId">Identifier of previous item, if known</param>
        /// <returns>Identifier for package element.</returns>
        private string ParseMsuPackageElement(XmlNode node, ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId)
        {
        ...
        private string ParseChainPackage(XmlNode node, ChainPackageType packageType, ComplexReferenceParentType parentType, string parentId, ComplexReferenceChildType previousType, string previousId)
        {
            ...
            string generatedId = this.core.GenerateIdentifier("pkg", packageType.ToString(), (null != sourceFile) ? sourceFile.ToUpperInvariant() : String.Empty);
            ...
            if (!this.core.EncounteredError)
            {
                ...
                this.CreatePayloadRows(sourceLineNumbers, generatedId, name, sourceFile, downloadUrl,
                    ComplexReferenceParentType.Package, id, ComplexReferenceChildType.Unknown, null);
        ...
        {
            Debug.Assert(ComplexReferenceParentType.Unknown == parentType || ComplexReferenceParentType.PackageGroup == parentType);
            Debug.Assert(ComplexReferenceChildType.Unknown == previousType || ComplexReferenceChildType.PackageGroup == previousType || ComplexReferenceChildType.Package == previousType);
