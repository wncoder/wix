...
namespace Microsoft.Tools.WindowsInstallerXml
{
    ...
    public sealed class Compiler
    {
        ...
        private string ParseComponentElement(XmlNode node, ComplexReferenceParentType parentType, string parentId, string parentLanguage, int diskId, string directoryId, string srcPath)
        {
            ...
            bool keyFound = false;
            string keyPath = null;
            bool keyPathContainsProperties = false;
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (XmlNodeType.Element == child.NodeType)
                {
                    ...
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        switch (child.LocalName)
                        {
                            ...
                            case "Registry":
                                ...
                                break;
                            case "RegistryKey":
                                keyPathSet = this.ParseRegistryKeyElement(child, id, CompilerCore.IntegerNotSet, null, win64, out keyPossible, out containsProperties);
                                keyBit = MsiInterop.MsidbComponentAttributesRegistryKeyPath;
                                break;
                            case "RegistryValue":
                                keyPathSet = this.ParseRegistryValueElement(child, id, CompilerCore.IntegerNotSet, null, win64, out keyPossible, out containsProperties);
                                keyBit = MsiInterop.MsidbComponentAttributesRegistryKeyPath;
                                break;
                            case "RemoveFile":
                            ...
                            case "ServiceInstall":
                                ...
                                break;
                            case "Shortcut":
                                this.ParseShortcutElement(child, id, node.LocalName, directoryId, false);
                                break;
                            case "SymbolPath":
                                if (null != symbols)
                                {
                    ...
                    else
                    {
                        ...
                        keyPathSet = CompilerExtension.ComponentKeypathType.None != keyType;
                        containsProperties = CompilerExtension.ComponentKeypathType.RegistryFormatted == keyType;
                    ...
                    if (null != keyPossible && (keyPathSet || (null == keyPath && !keyFound)))
                    {
                        keyFound = keyPathSet;
                        keyPath = keyPossible;
                        keyPathContainsProperties = containsProperties;
                        keyBits = keyBit;
                    }
                }
            }
            ...
            if ("*" == guid)
            {
                ...
                if (encounteredODBCDataSource ||
                    (1 != files && 0 == keyBits) ||
                    (0 != files && MsiInterop.MsidbComponentAttributesRegistryKeyPath == keyBits) ||
                    (0 == files && MsiInterop.MsidbComponentAttributesRegistryKeyPath == keyBits && keyPathContainsProperties))
                {
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (XmlNodeType.Element == child.NodeType)
                {
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        ...
                        {
                            case "Shortcut":
                                this.ParseShortcutElement(child, componentId, node.LocalName, directoryId, false);
                                break;
                            case "Permission":
        ...
        /// <param name="containsProperties">Flag specifying that the registry key has at least one embedded Property (i.e. Key="[Property]").</param>
        ...
        private bool ParseRegistryKeyElement(XmlNode node, string componentId, int root, string parentKey, bool win64Component, out string possibleKeyPath, out bool containsProperties)
        {
            SourceLineNumberCollection sourceLineNumbers = Preprocessor.GetSourceLineNumbers(node);
            string id = null;
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (XmlNodeType.Element == child.NodeType)
                {
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        ...
                        bool childContainsProperties = false;

                        switch (child.LocalName)
                        {
                            case "RegistryKey":
                                if (this.ParseRegistryKeyElement(child, componentId, root, key, win64Component, out possibleChildKeyPath, out childContainsProperties))
                                {
                                    if (keyPath)
                                    {
                                    ...
                                    containsProperties = childContainsProperties;
                                    keyPath = true;
                                }
                                ...
                                break;
                            case "RegistryValue":
                                if (this.ParseRegistryValueElement(child, componentId, root, key, win64Component, out possibleChildKeyPath, out childContainsProperties))
                                {
                                    if (keyPath)
                                    {
                                        this.core.OnMessage(WixErrors.ComponentMultipleKeyPaths(sourceLineNumbers, child.Name, "KeyPath", "yes", "File", "RegistryValue", "ODBCDataSource"));
        ...
        [SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase", Justification = "Changing the way this string normalizes would result " +
                         "in a change to the way the Registry table is generated, potentially causing extra churn in patches on an MSI built from an older version of WiX. " +
                         "Furthermore, there is no security hole here, as the strings won't need to make a round trip")]
        private bool ParseRegistryValueElement(XmlNode node, string componentId, int root, string parentKey, bool win64Component, out string possibleKeyPath, out bool containsProperties)
        {
            SourceLineNumberCollection sourceLineNumbers = Preprocessor.GetSourceLineNumbers(node);
            string id = null;
            string key = parentKey; // default to parent key path
            ...
            if (!this.core.EncounteredError)
            {
                ...
                containsProperties = Common.ContainsProperty(String.Concat(key, "\\", name));
            }
        ...
        [SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase", Justification = "Changing the way this string normalizes would result " +
                         "in a change to the way the Registry table is generated, potentially causing extra churn in patches on an MSI built from an older version of WiX. " +
                         "Furthermore, there is no security hole here, as the strings won't need to make a round trip")]
        private bool ParseRegistryElement(XmlNode node, string componentId, int root, string parentKey, bool win64Component, out string possibleKeyPath)
        {
            SourceLineNumberCollection sourceLineNumbers = Preprocessor.GetSourceLineNumbers(node);
            string id = null;
            string key = parentKey; // default to parent key path
                    ...
                    if (child.NamespaceURI == this.schema.TargetNamespace)
                    {
                        switch (child.LocalName)
                        {
                            ...
                            case "Registry":
                                ...
                                string possibleChildKeyPath = null;
                                bool childIsKeyPath = this.ParseRegistryElement(child, componentId, root, key, win64Component, out possibleChildKeyPath);
                                if (childIsKeyPath)
                                {
                                    if (keyPath)
                                    {
                ...
                if (0 == attrib.NamespaceURI.Length || attrib.NamespaceURI == this.schema.TargetNamespace)
                {
                    switch (attrib.LocalName)
                    {
                        ...
                        case "PreShutdownDelay":
                            preShutdownDelay = this.core.GetAttributeValue(sourceLineNumbers, attrib, true);
                            break;
                        case "ServiceName":
                ...
                if (0 == attrib.NamespaceURI.Length || attrib.NamespaceURI == this.schema.TargetNamespace)
                {
                    switch (attrib.LocalName)
                    {
                        case "Id":
                            ...
                            break;
                        case "Command":
                            command = this.core.GetAttributeValue(sourceLineNumbers, attrib, true);
                            break;
                        case "OnInstall":
                            YesNoType install = this.core.GetAttributeYesNoValue(sourceLineNumbers, attrib);
                            if (YesNoType.Yes == install)
                        ...
                        case "OnUninstall":
                            ...
                            break;
                        case "RebootMessage":
                            rebootMessage = this.core.GetAttributeValue(sourceLineNumbers, attrib, true);
                            break;
                        case "ResetPeriod":
        ...
        private void ParseShortcutElement(XmlNode node, string componentId, string parentElementLocalName, string defaultTarget, bool parentKeyPath)
        {
            SourceLineNumberCollection sourceLineNumbers = Preprocessor.GetSourceLineNumbers(node);
            string id = null;
            ...
            if (!this.core.EncounteredError)
            {
                ...
                if (advertise)
                {
                    if (!parentKeyPath && "Component" != parentElementLocalName)
                    {
