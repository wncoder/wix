    ...
    public sealed class Linker : IMessageHandler
    {
            ...
            try
            {
                ...
                foreach (Section section in output.Sections)
                {
                    ...
                    foreach (Table table in section.Tables)
                    {
                        ...
                        bool copyRows = true;
                        if (this.dropUnrealTables && table.Definition.IsUnreal && !table.Definition.IsUxManifest)
                        {
                            copyRows = false;
                        }
        ...
        */
        ...
        private void FlattenBundleTables(Output output)
        {
            ...
            groups.UseTypes(new string[] { "PayloadGroup", "Package" }, new string[] { "Payload", "PayloadGroup" });
            groups.FlattenAndRewriteRows("PayloadGroup", "WixUXContainer", false);
