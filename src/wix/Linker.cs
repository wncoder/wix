            ...
            try
            {
                ...
                foreach (Section section in output.Sections)
                {
                    ...
                    foreach (Table table in section.Tables)
                    {
                        ...
                        // or if user does not request dropping unreal tables from the output
                        bool copyRows = true;
                        if (this.dropUnrealTables && table.Definition.IsUnreal)
                        {
                            copyRows = false;
                        }
                        ...
                        switch (table.Name)
                        {
                            ...
                            case "Directory":
                                ...
                                {
                                    if (OutputType.Module == this.activeOutput.Type && Util.IsStandardDirectory(row[0].ToString()))
                                    {
