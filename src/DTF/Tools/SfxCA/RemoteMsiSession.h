//---------------------------------------------------------------------
// <copyright file="RemoteMsiSession.h" company="Microsoft">
...
private:
	...
	DWORD StartProcessingReqests()
	{
		...
		else
		{
			...
			m_hReceiveThread = CreateThread(NULL, 0,
				RemoteMsiSession::ProcessRequestsThread, this, 0, NULL);
	...
	//
	static DWORD WINAPI ProcessRequestsThread(void* pv)
	{
		RemoteMsiSession* pThis = (RemoteMsiSession*) pv;
		...
		while (pThis->m_fConnected)
		{
			RequestId id;
			RequestData req;
			dwRet = pThis->ReceiveRequest(&id, &req);
			if (dwRet != 0)
			{
			...
			{
				RequestData resp;
				pThis->ProcessRequest(id, &req, &resp);
				if (id == EndSession)
				{
					break;
				}

				dwRet = pThis->SendResponse(id, &resp);
	...
	//
	DWORD SendResponse(RequestId id, RequestData* pResp)
	{
	...
	DWORD ReadRequestData(RequestData* pReq)
	{
		...
		if (dwRet == 0)
		{
			...
			if (cbData > 0)
			{
				...
				dwRet = this->ReadPipe((BYTE*) m_pBuf, cbData);
				if (dwRet == 0)
				{
					DWORD dwOffset = 0;
					for (int i = 0; i < MAX_REQUEST_FIELDS; i++)
	...
	BOOL CheckRequestDataBuf(DWORD cbBuf)
	{
		if (m_cbBuf < cbBuf)
		{
			...
			m_cbBuf = max(1024, cbBuf);
			m_pBuf = new BYTE[m_cbBuf];
			if (m_pBuf == NULL)
			{
				m_cbBuf = 0;
			}
...
private:
	...
	// Dynamically-resized buffer for sending and receiving requests.
	BYTE* m_pBuf;
