...
public class InstallPackage : Database
{
    ...
    {
        longFileName = longFileName.ToLower(CultureInfo.InvariantCulture);
        ArrayList fileList = new ArrayList();
        foreach(KeyValuePair<string, InstallPath> entry in this.Files)
        {
            if(((InstallPath) entry.Value).TargetName.ToLower(CultureInfo.InvariantCulture)
               == longFileName)
            {
        ...
        InstallPathMap compressedFileMap, InstallPathMap uncompressedFileMap)
    {
        ...
        foreach(InstallPath fileInstallPath in uncompressedFileMap.Values)
        {
            ...
            if(Path.GetFullPath(sourcePath).ToLower(CultureInfo.InvariantCulture) !=
                Path.GetFullPath(extractPath).ToLower(CultureInfo.InvariantCulture))
            {
