    ...
    )
{
    ...
    // Check for presence of ngen.exe
    f32NgenExeExists = FileExistsEx(pwz32Ngen, NULL);
    f64NgenExeExists = FileExistsEx(pwz64Ngen, NULL);

    if (!f32NgenExeExists)
    {
        WcaLog(LOGMSG_TRACEONLY, "%S does not exist. No actions will be scheduled for 32bit Ngen.", pwz32Ngen);
    }

    if (!f64NgenExeExists)
    {
        WcaLog(LOGMSG_TRACEONLY, "%S does not exist. No actions will be scheduled for 64bit Ngen.", pwz64Ngen);
    }
