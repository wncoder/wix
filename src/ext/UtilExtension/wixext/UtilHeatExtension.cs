...
namespace Microsoft.Tools.WindowsInstallerXml.Extensions
{
    ...
    public sealed class UtilHeatExtension : HeatExtension
    {
        ...
        public override HeatCommandLineOption[] CommandLineTypes
        {
            get
            {
                return new HeatCommandLineOption[]
                {
                    ...
                    new HeatCommandLineOption("-var <VariableName>", "substitute File/@Source=\"SourceDir\" with a preprocessor variable" + Environment.NewLine +
                    ...
                    new HeatCommandLineOption("-t:<xsl>", "transform harvested output with XSL file"),
        ...
        public override void ParseOptions(string type, string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                if ('-' == commandSwitch[0] || '/' == commandSwitch[0])
                {
                    ...
                    else if (truncatedCommandSwitch.StartsWith("t:", StringComparison.Ordinal))
                    {
                        string xslFile = truncatedCommandSwitch.Substring(2);
                    ...
                    else if (truncatedCommandSwitch.StartsWith("template:", StringComparison.Ordinal))
                    {
                        switch (truncatedCommandSwitch.Substring(9))
                        {
                            case "fragment":
        ...
        private string GetArgumentParameter(string[] args, int index)
        {
            ...
            {
                if (commandSwitchValue.Contains(" "))
                {
