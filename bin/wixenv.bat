...
:WOW64KEY
set REGKEY=HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\VisualStudio\9.0\Setup\VC
goto GETVCDIR

:REGULARKEY
set REGKEY=HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio\9.0\Setup\VC
goto GETVCDIR

:GETVCDIR
set VCDIR=
FOR /F "tokens=2* delims=	 " %%A IN ('REG QUERY "%REGKEY%" /v ProductDir') DO SET VCDir=%%~sB
if not exist "%VCDIR%vcvarsall.bat" echo ERROR: 'vcvarsall.bat' file not found. Please ensure the Windows Server 2008 and .NET Framework 3.5 SDK is installed, or manually call this batch file to configure the build tool paths.
if exist "%VCDIR%vcvarsall.bat" call "%VCDIR%vcvarsall.bat"
call :CLEANPATH
